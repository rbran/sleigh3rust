...
mod test {
    ...
    impl GlobalSetTrait for GlobalSetDummy {
        ...
        fn set_cond_true(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }

        fn set_itmode(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }
        ...
        fn set_cond_shft(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }
        ...
        fn set_TEEMode(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }
        ...
        fn set_cond_full(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }
        ...
        fn set_cond_mask(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }

        fn set_condit(&mut self, address: Option<u32>, value: u16) {
            todo!()
        }
        ...
        fn set_cond_base(&mut self, address: Option<u32>, value: u8) {
            todo!()
        }
    }
    ...
    fn disassembler_simple1() {
        let tokens_be: &[(u32, &str, &[u8])] = &[
            ...
            (0x1000c, "mov r0,r0", &[0xe1, 0xa0, 0x00, 0x00]),
        ];
        let be_context = Context {
            register: ContextSpaceregister {
                chunk_0x0: [0u8; 8],
            },
        };
        ...
        for &(addr, output, token) in tokens_be.iter() {
            ...
            let parsed =
                instruction::parse(token, &mut context, addr, &mut global_set);
            match parsed {
                None => panic!("Instruction invalid"),
                Some((_rest, instruction)) => {
                    let mut display = Vec::new();
                    instruction.display_extend(&mut display, &context);
                    let display: String =
                        display.drain(..).map(|x| x.to_string()).collect();
